  ...
  void _getTxDigestForCurrentwallet() async {
    ...
    try {
      if (transactions.isNotEmpty) {
        ...
        suiTansactionList = _getFakeTransactions();
      }
    } catch (e) {
      suiTansactionList = [];
    }
    suiTansactionListSend = suiTansactionList?.where((element) => element.isSender) as List<SuiTansaction>;
    suiTansactionListReceive = suiTansactionList?.where((element) => !element.isSender) as List<SuiTansaction>;
    setState(() {});
  }
              ...
              child: TabBarView(
                ...
                children: tabs.map((e) {
                  if (e == "Info") {
                    ...
                    return getListView(e == "Send" ? suiTansactionListSend : suiTansactionListReceive);
                  } else {
                    return Container();
                  }
                }).toList(),
                ...
                Column(
                  ...
                  children: [
                    Container(
                      alignment: Alignment.centerLeft,
                      child: Text(
                        !dataList[index].kind.contains("Pay")
                            ? dataList[index].kind
                            : ((dataList[index].isSender ? "-" : "+") +
                            (NumUtil.getNumByValueDouble(neverDoubleTryOrZero(dataList[index].amount.toString()) / 1000000000, 9)).toString()),
